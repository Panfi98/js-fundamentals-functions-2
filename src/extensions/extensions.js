...
function calculateQuantities(ingredients, layers) {
  anObj = { sugar: 0, eggs: 0 }

  anObj.sugar += ingredients.includes('sugar')? 100 * layers : 0
  anObj.eggs += ingredients.includes('eggs')? 2 * layers: 0
...
function improveRecipe(objRecepie, portions){
  Object.entries(objRecepie).forEach(([key, value]) => {
    objRecepie[key] = value * portions
  });
  return objRecepie
}
